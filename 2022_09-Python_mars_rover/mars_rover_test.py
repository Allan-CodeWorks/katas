...
def test_navigate_no_instructions(navigate_input_position, expected):
    assert(navigate(navigate_input_position, [], []) == expected)
...
def test_simple_forward_east(navigate_input_position, expected):
    assert(navigate(navigate_input_position, ['Forward'], []) == expected)
...
positions_test_double_forward_west = [
    {'x': 5, 'y': 1, 'direction': 'W'},
    {'x': 6, 'y': 1, 'direction': 'W'},
    {'x': 7, 'y': 1, 'direction': 'W'}
]
...
positions_test_double_forward_west_expected = [
    {'x': 3, 'y': 1, 'direction': 'W'},
    {'x': 4, 'y': 1, 'direction': 'W'},
    {'x': 5, 'y': 1, 'direction': 'W'}
]
...
@pytest.mark.parametrize('navigate_input_position, expected',
                         zip(positions_test_double_forward_west,
                             positions_test_double_forward_west_expected))
def test_double_forward_west(navigate_input_position, expected):
    assert(navigate(navigate_input_position, [
...
@pytest.mark.parametrize('navigate_input_position, expected',
                         zip(positions_test_simple_turn_rigth,
                             positions_test_simple_turn_rigth_expected))
def test_simple_turn_rigth(navigate_input_position, expected):
    assert(navigate(navigate_input_position, [
           'Turn Right', ], []) == expected)
